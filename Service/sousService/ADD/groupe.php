...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $nomUnite = htmlspecialchars($_POST["groupes"], ENT_QUOTES, 'UTF-8');
  if (!empty($nomUnite)) {
      
      $checkIfExists = "SELECT COUNT(*) AS count FROM groupes WHERE BINARY nom = '$nomUnite'";
       ...
       if ($row['count'] == 0) {
           ...
           $sql = "INSERT INTO groupes (nom) VALUES ('$nomUnite')";
          
           if ($connexion->query($sql) === TRUE) {
              echo "<script>alert('Le groupe a été ajoutée avec succès à la base de données.');</script>";
           } else {
             echo "<script>alert('Erreur lors de l\'ajout du groupe : " . $connexion->error . "');</script>";
           }
       } else {
           echo "<script>alert('Un groupe avec le même nom existe déjà.');</script>";
       }

       
       $connexion->close();
  } else {
      echo "<script>alert('Le champ du nom du groupe ne peut pas être vide.');</script>";
  }
}
?>
        <div class="logo">
    ...
    <div class="cont">
        <div class="titre">ADD Groupe</div>
        ...
        <form action="groupe.php" method="post">
            ...
            <span class="name1"> <input type="text" name="groupes"></span>
