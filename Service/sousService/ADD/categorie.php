...
if ($conn->connect_error) {
    ...
    $query_groups = "SELECT nom FROM groupes";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $query_check_category = "SELECT id FROM categorie WHERE nom = '$name'";
        $result_check_category = $conn->query($query_check_category);

        if ($result_check_category->num_rows > 0) {
            ?>
            <script>
                alert("La catégorie existe déjà. Vous ne pouvez pas ajouter une catégorie existante.");
            </script>
            <?php
        } else {
            // Récupérer l'ID du groupe depuis la base de données
            $query_group_id = "SELECT id FROM groupes WHERE nom = '$group'";
            $result_group_id = $conn->query($query_group_id);

            if ($result_group_id !== false) {
                $row_group_id = $result_group_id->fetch_assoc();
                $group_id = $row_group_id['id'];

                // Insérer la catégorie dans la table categorie
                $sql = "INSERT INTO categorie (nom, id_Groupe) VALUES ('$name', $group_id)";
                ...
                if ($conn->query($sql) === TRUE) {
                    ...
                    ?>
                    <script>
                        alert("Erreur lors de l'insertion de la catégorie : <?php echo $conn->error; ?>");
                    </script>
                    <?php
                }
            } else {
                ?>
                <script>
                    alert("Erreur lors de la récupération de l'ID du groupe depuis la base de données : <?php echo $conn->error; ?>");
                </script>
                <?php
            }
        }
...
?>
    ...
    <div class="cont">
        <div class="titre">ADD Categorie</div>
