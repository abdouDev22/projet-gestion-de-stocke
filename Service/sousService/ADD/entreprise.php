...
<body>
    ...
    <div class="cont">
        <div class="titre">ADD Entreprise </div>
        ...
        <form action="entreprise.php" method="post">
            ...
            <span class="pri1"><input type="text" name="adress"></span>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $tel = $_POST["tel"];
    $adress = $_POST["adress"];
    if (empty($name) || empty($tel) || empty($adress)) {
      ?>
      <script>
      ...
      $query_check_client = "SELECT id FROM Client_Fourniseur WHERE nom = '$name'";
     $result_check_client = $conn->query($query_check_client);
  
      if ($result_check_client->num_rows > 0) {
        ?>
              <script>
                  alert("L\'entreprise existe déjà. Vous ne pouvez pas ajouter une entreprise existant.");
              </script>
              <?php
      } else {
          // Insérer le client dans la table Client_Fourniseur
          $sql = "INSERT INTO Client_Fourniseur (nom, adresse, telephone, Type) VALUES ('$name', '$adress', '$tel', 'fournisseur')";
  
          if ($conn->query($sql) === TRUE) {
            ?>
              <script>
                  alert("Entreprise ajouté avec succès.");
              </script>
              <?php
          } else {
            ?>
              <script>
                  alert("Erreur lors de l'insertion de l\'entreprise :  <?php $conn->error  ?>");
              </script>
              <?php
          }
      }
