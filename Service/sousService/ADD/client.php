...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($tel)) {
      ...
      ?>
      <script>
          alert("Le nom et le telephone ne peux pas etre vide ");
      </script>
      <?php
    }else{
 // Vérifier si le client existe déjà
 $query_check_client = "SELECT id FROM Client_Fourniseur WHERE nom = '$name' AND telephone = '$tel'";
 $result_check_client = $conn->query($query_check_client);

 if ($result_check_client->num_rows > 0) {
   ?>
         <script>
             alert("Le client existe déjà. Vous ne pouvez pas ajouter un client existant.");
         </script>
         <?php
 } else {
     // Insérer le client dans la table Client_Fourniseur
     $sql = "INSERT INTO Client_Fourniseur (nom, adresse, telephone, Type) VALUES ('$name', '$adress', '$tel', 'client')";
     ...
     if ($conn->query($sql) === TRUE) {
       ...
       ?>
         <script>
             alert("Erreur lors de l'insertion du client :  <?php $conn->error  ?>");
         </script>
         <?php
     }
 }
...
?>
    ...
    <div class="cont">
        <div class="titre">Add Client</div>
