...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $nomUnite = htmlspecialchars($_POST["nom_unite"], ENT_QUOTES, 'UTF-8');
  if (!empty($nomUnite)) {
      
      $checkIfExists = "SELECT COUNT(*) AS count FROM UnitesVente WHERE BINARY nom = '$nomUnite'";
      
       $result = $connexion->query($checkIfExists);
       $row = $result->fetch_assoc();
      
       if ($row['count'] == 0) {

           $sql = "INSERT INTO UnitesVente (nom) VALUES ('$nomUnite')";
          
           if ($connexion->query($sql) === TRUE) {
              echo "<script>alert('L\'unité a été ajoutée avec succès à la base de données.');</script>";
           } else {
             echo "<script>alert('Erreur lors de l\'ajout de l\'unité : " . $connexion->error . "');</script>";
           }
       } else {
           echo "<script>alert('Une unité avec le même nom existe déjà.');</script>";
       }

       
       $connexion->close();
  } else {
      echo "<script>alert('Le champ du nom de l\'unité ne peut pas être vide.');</script>";
  }
}
?>
    <nav>
    ...
    <div class="cont">
        ...
        <form action="unite.php" method="POST">
            ...
            <span class="name1"> <input type="text" name="nom_unite"></span>
