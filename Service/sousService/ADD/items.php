...
if ($conn->connect_error) {
    ...
    $query_units = "SELECT nom FROM UnitesVente";
    $query_categories = "SELECT nom FROM categorie";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $query_check_product = "SELECT id FROM Produits WHERE nom = '$name'";
        $result_check_product = $conn->query($query_check_product);

        if (!empty($name)&&!empty($quantity)&&!empty($price)&&!empty($unit)&&!empty($category)) {
          if ($result_check_product->num_rows > 0) {
            ?>
            <script>
                alert("Le produit existe déjà. Vous ne pouvez pas ajouter un produit existant.");
            </script>
            <?php
        } else {
            // Récupérer l'ID de l'unité depuis la base de données
            $query_unit_id = "SELECT id FROM UnitesVente WHERE nom = '$unit'";
            $result_unit_id = $conn->query($query_unit_id);

            // Récupérer l'ID de la catégorie depuis la base de données
            $query_category_id = "SELECT id FROM Categorie WHERE nom = '$category'";
            $result_category_id = $conn->query($query_category_id);

            if ($result_unit_id !== false && $result_category_id !== false) {
                $row_unit_id = $result_unit_id->fetch_assoc();
                $unit_id = $row_unit_id['id'];

                $row_category_id = $result_category_id->fetch_assoc();
                $category_id = $row_category_id['id'];

                // Insérer le produit dans la table Produits
                $sql = "INSERT INTO Produits (nom, quantite, prix, Id_UnitesVente, id_categorie, date_ajout) VALUES ('$name', $quantity, $price, $unit_id, $category_id, NOW())";
                ...
                if ($conn->query($sql) === TRUE) {
                    ...
                    ?>
                    <script>
                        alert("Erreur lors de l'insertion du produit : <?php echo $conn->error; ?>");
                    </script>
                    <?php
                }
            } else {
                ?>
                <script>
                    alert("Erreur lors de la récupération des ID depuis la base de données : <?php echo $conn->error; ?>");
                </script>
                <?php
            }
        }
      } else {
        echo "<script>alert('Les champs du produit  ne peut pas être vide.');</script>";
    }
    }
}
?>
    ...
    <div class="cont">
        <div class="titre">ADD items</div>
        <a href="../../index.php" onclick="local()" id="ww" class="lien-ret"><svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 24 24" fill="none"><path d="M16 12H8M8 12L11 9M8 12L11 15" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/></svg></a>
        <form action="items.php" method="post">
    <label for="name" class="name">Name</label>
    <span class="name1"> <input type="text" name="name"></span>

    <label for="quantity" class="qua">Quantité :</label>
    <span class="qua1"><input type="text" name="quantity"></span>

    <label for="price" class="pri">Prix :</label>
    <span class="pri1"><input type="text" name="price"></span>

    <label for="unit" class="uni">Unite :</label>
    <span class="uni1">
        <select name="unit" id="">
            <?php
            if ($result_units) {
                while ($row = $result_units->fetch_assoc()) {
                    $selected = ($unit == $row['nom']) ? 'selected' : '';
    ...
    <label for="category" class="qq">Categories :</label>
    <span class="qq1">
        <select name="category" id="">
            <?php
            if ($result_categories) {
                while ($row = $result_categories->fetch_assoc()) {
                    $selected = ($category == $row['nom']) ? 'selected' : '';
                    echo "<option value='{$row['nom']}' $selected>{$row['nom']}</option>";
                }
            }
            ?>
