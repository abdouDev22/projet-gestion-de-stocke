...
<head>
    ...
    <link rel="stylesheet" href="../../../asset/rebourser1.css">
...
if ($conn->connect_error) {
    ...
    $query_clients = "SELECT DISTINCT
    u.nom as nom_client, 
    SUM(pc.quantite) as quantite_totale,
    c.id as id_commande,
    Statut_transaction,c.id_utilisateur 
FROM Commandes c
JOIN client_fourniseur u ON c.id_Client_Fourniseur = u.id
JOIN produits_commande pc ON c.id = pc.id_commande
    ...
    <ul>
      ...
      <li class="list">
        <a href="../../index.php" id="ll">
          <span class="icon"><svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    while($row = $result_clients->fetch_assoc()) {
        echo '<div class="item">
                <div class="item-content">
                    ...
                    <span><a href="traitementDay.php?id=' . $row["id_commande"] . '" class="modifier">Traitement</a></span>
                </div>
            </div>';
    }
}else {
...
filterButton1.addEventListener('keyup', function () {
  ...
  if (grid && titre) {
    ...
    for (var i = 0; i < items.length; i++) {
      let titleElement = items[i].getElement().querySelector('.no1');
      let numElement = items[i].getElement().querySelector('.type1');

      if (titleElement) {
        var title = titleElement.textContent.toLowerCase();
        var num = numElement.textContent.toLowerCase(); 
        if (title.includes(searchTerm) || num.includes(searchTerm)) {
