...
if ($conn->connect_error) {
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST["nome"])&&isset($_POST["Date_Naissance"])&&isset($_POST["tel"])&&isset($_POST["email"])&&isset($_POST["poste"])&&isset($_POST["Address"])){
      $name = $_POST["nome"];
      ...
      $sql = "UPDATE Utilisateurs SET nomcomplet='$name', date_naissance='$Date_Naissance', adresse='$address', telephone='$tel1', email='$email1', post='$poste' WHERE id=$id_utilisateur";
      $conn->query($sql);
...
?>
  ...
  <div class="sidebar">
    ...
    <ul>
      ...
      <li class="list">
        <a href="../../index.php"><span class="icon"><svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15 12L12 12M12 12L9 12M12 12L12 9M12 12L12 15" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/><path d="M22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C21.5093 4.43821 21.8356 5.80655 21.9449 8" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/></svg></span>
  ...
  <form action="profile.php" method="post">
    <div class="cont">
      <div class="log"><a class="util" href="#">Info-log</a></div>
      <div class="name">
        Full Name
      </div>
      <div class="name1">
        <input type="text" value="<?php echo $nom  ?>" name="nome" required>
      </div>
      <div class="Date-Naissance">
        Date de Naissance
      </div>
      <div class="Date-Naissance1">
        <input type="datetime" value="<?php echo $date  ?> " name="Date_Naissance" required>
      </div>
      <div class="Address">
        Adress
      </div>
      ...
      <div class="Tel1"><input type="text" value="<?php echo $tel  ?> " name="tel" required></div>
      <div class="Email">Email</div>
      <div class="Email1"><input type="email" value="<?php echo $email ?>" name="email" required></div>
      <div class="post">POSTE</div>
      <div class="post1"><input type="text" value="<?php echo $poste ?>" name="poste" required></div>
      <div class="chang"><a href="#" onclick="validateForm()"><button type="submit">Changer</button></a></div>
    </div>
  </form>
  

  <form action="utilisateur.php" method="post" id="form2">
    <div class="cont1">
      <a href="#" id="zz" class="lien-ret"><svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 24 24" fill="none"><path d="M16 12H8M8 12L11 9M8 12L11 15" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/></svg></a>
      <h1>Information sur l'utisateur</h1>
      <div class="case1"><input type="checkbox" value="name" name="name_check" id=""></div>
      <div class="gen user">Username</div>
      <div class="gen user1"><input type="text" value="<?php echo $username  ?>" name="u_name" readonly required></div>
      <div class="case2"><input type="checkbox" value="pass" name="name_check" id=""></div>
      <div class="gen1 pass">Password</div>
      <div class="gen1 pass1"><input type="password"  value="<?php echo $pass  ?>" name="u_pass" readonly required></div>
      <div class="gen1 cofpass">Confirmer Password</div>
      <div class="gen1 cofpass1"><input type="password" name="u_pass_conf" ></div>
      <div class="chang1" ><a  href="#" ><button>Changer</button></a></div>
    </div>
  </form>
  ...
  <script>
const tt = document.querySelector('.util');
const tt1 = document.querySelector('.lien-ret');
const cont = document.querySelector('.cont');
const cont1 = document.querySelector('.cont1');
const ultilisateurInput=document.querySelector('.case1 input')
const passInput=document.querySelector('.case2 input')
const gens=document.querySelectorAll('.gen')
const gens1=document.querySelectorAll('.gen1')
const password=document.querySelector('.pass1 input')
const change=document.querySelector('.chang1 a')
const formulaire = document.getElementById('form2');

formulaire.addEventListener('submit',(event)=>{
  if (!passInput.checked && !ultilisateurInput.checked) {
    alert("Veuillez sélectionner le champ que vous voulez changer.");
...
passInput.addEventListener("click", () => {
  if (passInput.checked) {
    gens1[0].style.opacity = '1';
    gens1[1].style.opacity = '1';
    password.readOnly = false;
    password.addEventListener("input",() =>{
      gens1[2].style.opacity = '1';
      gens1[3].style.opacity = '1';
    })

  } else { 
    password.readOnly = true;
    gens1[0].style.opacity = '0.2';
    gens1[1].style.opacity = '0.2';
    gens1[2].style.opacity = '0';
      gens1[3].style.opacity = '0';
  }
});

ultilisateurInput.addEventListener("click", () => {
  if(ultilisateurInput.checked==1){
    gens[1].querySelector('input').readOnly = false;
    ...
    gens[1].querySelector('input').readOnly = true;
    for(const gen of gens){
      gen.style.opacity='0.2'
    }
  }
});
...
tt.addEventListener("click", () => {
  ...
  cont.style.top = '-100%';
  cont.style.transition = '10s top, 0.5s opacity';

  cont1.style.transition = '0.5s left, 2s top, 4s opacity';
  cont1.style.transitionDelay = '1s';
  cont1.style.top = '20%';
  cont1.style.left = '5%';
...
tt1.addEventListener("click", () => {
  ...
  cont.style.top = '20%';
  cont.style.transition = '2.5s';
  cont.style.transitionDelay = '1s';

  cont1.style.transition = '35s left, 2s top, 1s opacity';
  cont1.style.top = ' 150%';
  cont1.style.left = '155%';
  ...
  <script>
    ...
    // Ajoute un écouteur d'événements click à chaque élément de la liste
    for (let i = 0; i < list.length; i++) {
      list[i].onclick = function () {
        // Supprime la classe active de toutes les listes
        for (let j = 0; j < list.length; j++) {
          list[j].className = 'list';
        }
    
        // Ajoute la classe active à la liste cliquée
        this.className = 'list active';
    
        // Stocke l'id de la liste cliquée dans SessionStorage
        sessionStorage.setItem("activeList", this.id);
      };
    }
    </script>
 <script>
