...
?>
  ...
  <div class="cont1 n">
    ...
    <h1>Emprunter</h1>
    <a href="../clientEmprunt.php" onclick="local()" class="lien-ret"><svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 24 24" fill="none"><path d="M16 12H8M8 12L11 9M8 12L11 15" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/></svg></a>
   
    <span class="Name">Name client</span>
    <span class="Name1"><input type="text" name="nom_client" value="<?php echo $nom_client  ?>" id=""></span>
    <span class="date">Date</span>
    <span class="date1"><input type="date" name="date_commande" value="<?php echo $dateActuelle  ?>"></span>
    <span class="num">NO :</span>
    <span class="num1"><input type="number" value="<?php echo $nextId   ?>" readonly></span>
    ...
    <span class="total"><input class="tt" name="p" type="text" value="0" readonly></span>
      ...
      <span class="vendre"><input type="submit" value="Emprunter"></span>
    </form>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  for ($i = 0; $i < count($_POST['nom_produit']); $i++) {
    ...
    // Vérifie si la quantité commandée est disponible
    if($quantite > $quantite_dispo) {
      ?>
      <script>
        alert("Erreur : Quantité demandée du produit <?php echo $nom_produit; ?> supérieure au stock disponible. quantite dispo <?php echo $quantite_dispo; ?>");
      </script>
      <?php
      $validation_quantite= false;
      continue; // Passe au prochain produit
    }
  
    // Si ok, insère dans la base de données
    // ...
...
if($validation_quantite) {
    ...
    $insert_commande = "INSERT INTO commandes (date_commande, id_utilisateur, id_Client_Fourniseur,quantite_totale_commande) 
                        VALUES ('$date_commande', $id_utilisateur, $id_client,$pr_total)";
    ...
    for ($i = 0; $i < count($_POST['nom_produit']); $i++) {
        ...
        if ($result_produit->num_rows > 0) {
            ...
            $insert_produits_commande = "INSERT INTO produits_commande (id_produit, id_commande, quantite, prix_unitaire, prix_total, Statut_transaction) 
                                         VALUES ($id_produit, $last_command_id, $quantite, $prix_unitaire, $prix_total, 'Emprunter')";
    ...
    ?>
            <script>
                 window.location.href="../clientAchat.php";      
            </script>
      <?php
  }
    // Ferme la connexion
...
<script>
  const prt=document.querySelector('#printBtn')
  prt.addEventListener('click',()=>{
    localStorage.setItem('name', document.querySelector('.Name1 input').value);
localStorage.setItem('date', document.querySelector('.date1 input').value);
localStorage.setItem('num', document.querySelector('.num1 input').value);
localStorage.setItem('total', document.querySelector('.tt').value);
// Retrieve the data from the table
const table = document.querySelector('.content-table');
const rows = table.querySelector('tbody');
const rowsString = rows.innerHTML;
localStorage.setItem('table',rowsString);
  })
