...
$id_commande=$_SESSION['Ater_cash'];

 $query = "SELECT commandes.date_commande,client_fourniseur.nom AS Nom_client,commandes.id,commandes.quantite_totale_commande AS p FROM commandes 
...
$query1= "SELECT produits_commande.prix_total AS prix_total,produits.nom AS nom_produits,produits_commande.quantite AS quantite,produits.prix AS Prix_unitaire FROM produits_commande
 JOIN produits ON produits.id = produits_commande.id_produit WHERE produits_commande.id_commande=$id_commande AND produits_commande.Statut_transaction='Emprunter'
";
 ...
 ?>
  ...
  <div class="cont1 n">
    ...
    <h1>RENDU d'emprunt</h1>
    <a href="Rembourser.php" onclick="local()" class="lien-ret"><svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 24 24" fill="none"><path d="M16 12H8M8 12L11 9M8 12L11 15" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/></svg></a>
    
    <span class="Name">Name client</span>
    <span class="Name1"><input type="text" name="nom_client" value="<?php echo $row['Nom_client']; ?>" id="" readonly></span>
    <span class="date">Date</span>
    <span class="date1"><input type="date" value="<?php echo $row['date_commande']; ?>" readonly></span>
    <span class="num">NO :</span>
    <span class="num1"><input type="number" value="<?php echo $id_commande; ?>" readonly></span>
    <span class="total1">Prix TOTAL</span>
    <span class="total"><input class="tt" type="text" name="p" value="0" readonly></span>
    <?php
        }
    } else {
        ?>
    ...
    ?>
      ...
      if ($result1->num_rows > 0) {
        while ($row = $result1->fetch_assoc()) {
            ?>
          <tr>
            <td><input type="text" name="nom_produit[]"  class="input" value="<?php echo $row['nom_produits']; ?>" readonly></td>
            <td><input type="number" value="<?php echo $row['quantite']; ?>" readonly></td>
            <td><input type="number" id="price" value="<?php echo $row['Prix_unitaire']; ?>" readonly></td>
            <td><input type="number" name="prix_total[]" value="<?php echo $row['prix_total']; ?>"  readonly></td>
          </tr>
          <?php
        }
    } 
    ?>
      ...
      <span class="vendre"><input type="submit" value="Valide"></span>
      <span class="inpu"><input type="number" name="hf" id="hf"></span>
      <span class="output" id="hh"><input type="number"></span>
      <span class="output1"><input type="checkbox"></span>
    </form>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    for ($i = 0; $i < count($_POST['nom_produit']); $i++) {
        ...
        if ($result_produit->num_rows > 0) {
            ...
            // mise a jour de la table produits_commande
            $up_produits_commande = "UPDATE produits_commande 
                         INNER JOIN commandes ON commandes.id = produits_commande.id_commande
                         SET produits_commande.Statut_transaction = 'vendre', 
                             commandes.quantite_totale_commande = commandes.quantite_totale_commande - $r
                         WHERE produits_commande.prix_total <= $r
                           AND produits_commande.id_commande = $id_commande
                           AND produits_commande.id_produit = $id_produit";


          

            if ($mysqli->query($up_produits_commande) !== TRUE) {
                ?>
                <script>
            ...
            <script>
                ...
                window.location.href="Rembourser.php";   
            </script>
            <?php
            }
        } else {
