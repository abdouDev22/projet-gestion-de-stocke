...
$query = "SELECT utilisateurs.nomcomplet AS nom_utilisateur,utilisateurs.id,roles_utilisateur.id, username, adresse,telephone  , roles_utilisateur.nom AS nom_role FROM utilisateurs, roles_utilisateur WHERE utilisateurs.id_role_utilisateur = roles_utilisateur.id AND utilisateurs.id = $idUnite";
...
if ($result->num_rows > 0) {
    ...
    $address=$row['adresse'];
    $type= $row['nom_role'];
    $query_unite = "SELECT * FROM roles_utilisateur";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $updateQuery = "UPDATE utilisateurs SET nomcomplet = '$nouveauNom', adresse='$adress',`telephone` ='$tel',username ='$usernme',id_role_utilisateur= (SELECT id FROM roles_utilisateur WHERE nom = '$type') WHERE id = $idUnite";
...
?>
    ...
    <div class="cont">
        <div class="titre">ALTER employe </div>
        <a href="../employe.php" onclick="local()" class="lien-ret"><svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 24 24" fill="none"><path d="M16 12H8M8 12L11 9M8 12L11 15" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/></svg></a>
        <form action="employe.php" method="post">
            <label for="" class="name">FULL NAME</label>
            <span class="name1"> <input type="text" value="<?php echo $nom ?>" name="nom"></span>
            <label for="" class="qua">Telephone :</label>
            <span class="qua1"><input type="number" value="<?php echo $tel ?>" name="tel"></span>
            <label for="" class="pri">Adresse :</label>
            <span class="pri1"><input type="text" value="<?php echo $address ?>" name="address"></span>
            <label for="" class="uni">UserName :</label>
            <span class="st"><input type="text" value="<?php echo $username ?>" name="usernme"></span>
            <label for="" class="t1">Type :</label>
            <span class="t"><select name="type" id="">
            <?php
            if ($result_unite) {
                while ($row2 = $result_unite->fetch_assoc()) {
                    $selected = ($type == $row2['nom']) ? 'selected' : '';
                    echo "<option value='{$row2['nom']}' $selected>{$row2['nom']}</option>";
                }
            } 
            ?>
            </select></span>
            <span class="reni"><a href="r.php?id=<?php echo $idUnite ?>">Renisiation Password</a></span>
