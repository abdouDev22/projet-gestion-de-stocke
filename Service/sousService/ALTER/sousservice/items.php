...
if(isset($_GET['id'])){
  $_SESSION['Ater_items']=$_GET['id'];
  $_SESSION['nom_cat']=$_GET['nom_cat'];
}

$idUnite = $_SESSION['Ater_items'];
$group=$_SESSION['nom_cat'];

// Récupérer les informations de l'unité avec l'ID spécifié
$query = "SELECT produits.nom AS nom_produits, quantite, prix, unitesvente.nom AS nom_unite FROM produits, unitesvente WHERE produits.Id_UnitesVente = unitesvente.id AND produits.id = $idUnite";

$query_group_id = "SELECT * FROM categorie ";
$result_groupe = $conn->query($query_group_id );
$result = $conn->query($query);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $nom = $row['nom_produits'];
    $quantie= $row['quantite'];
    $prix= $row['prix'];
    $unite= $row['nom_unite'];
    $query_unite = "SELECT * FROM unitesvente";
    $result_unite = $conn->query($query_unite);


} else {
    // Gérer le cas où l'unité avec l'ID spécifié n'existe pas
    echo "Aucune unité trouvée avec l'ID $idUnite.";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $updateQuery = "UPDATE produits SET nom = '$nouveauNom', quantite='$quantie',prix='$prix' ,Id_UnitesVente=(SELECT id FROM unitesvente WHERE nom = '$unites' LIMIT 1) , id_categorie = (SELECT id FROM categorie WHERE nom = '$categoris' LIMIT 1) WHERE id = $idUnite";

  if ($conn->query($updateQuery) === TRUE) {
    ?>
    <script>
        alert("information mise ajour");
...
?>
    ...
    <div class="cont">
        <div class="titre">Alter items</div>
        <a href="../items.php" id="ww" class="lien-ret"><svg xmlns="http://www.w3.org/2000/svg" width="800px" height="800px" viewBox="0 0 24 24" fill="none"><path d="M16 12H8M8 12L11 9M8 12L11 15" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round"/></svg></a>
        <form action="items.php" method="post"> 
            <label for="" class="name">Name</label>
            <span class="name1"> <input type="text" name="nom" value="<?php echo $row['nom_produits']; ?>"></span>
            <label for="" class="qua">Quantité :</label>
            <span class="qua1"><input type="text" name="quantite" value="<?php echo $quantie ?>"></span>
            <label for="" class="pri">Prix :</label>
            <span class="pri1"><input type="text" name="prix" value="<?php echo $prix ?>"></span>
            <label for="" class="uni">Unite :</label>
            <span class="uni1"><select name="unite" id="">
            <?php
            if ($result_unite) {
                while ($row2 = $result_unite->fetch_assoc()) {
                    $selected = ($unite == $row2['nom']) ? 'selected' : '';
            ...
            <label for="" class="qq">Categories :</label>
            <span class="qq1"><select name="categoris" id="">
            <?php
            if ($result_groupe) {
                while ($row1 = $result_groupe->fetch_assoc()) {
                    $selected = ($group == $row1['nom']) ? 'selected' : '';
                    echo "<option value='{$row1['nom']}' $selected>{$row1['nom']}</option>";
                }
            }
            ?>
            </select></span>
            <div class="chang"><button>ALTER</button></div>    
        </form>
      </div>
      ...
      <script>
        ...
        function local(){
          localStorage.setItem('nav-lien', 1);
        }
    ...
    listItems.forEach((item) => {
     ...
     <script>
      ...
      const sidebar1 = document.getElementsByClassName('mySidebar');
    icon.addEventListener('click', function() {
      sidebar.classList.toggle('blocked');
    });
