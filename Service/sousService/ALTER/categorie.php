...
<head>
    ...
    <link rel="stylesheet" href="../../../asset/cateR1.css">
...
if ($conn->connect_error) {
    ...
    $query_clients = "SELECT Categorie.nom AS nom_categorie, Groupes.nom AS nom_groupe,Categorie.id AS id
    FROM Categorie
    JOIN Groupes ON Categorie.id_Groupe = Groupes.id;
    " ;
    ...
    <nav>
      ...
      <div class="barre-plant">
        ...
        <h1>Alter Catégorie</h1>
        <input class="t" type="text" placeholder="Faites vos recherches ici">
      </div>
      <div class="grid">
      ...
      if ($result_clients->num_rows > 0) {
        while ($row = $result_clients->fetch_assoc()) {
            ...
            <div class="item-content">
             ...
             <span class="no1"><?php echo $row['nom_categorie']; ?></span>
             <span class="groupe">Groupe</span> 
             <span class="groupe1"><?php echo $row['nom_groupe']; ?></span>
             <span class="fff"><input type="number"></span>
              <span><a href="sousservice/categorie.php?id=<?php echo $row['id']; ?>&& nom_groupe=<?php echo $row['nom_groupe']; ?>" class="modifier">Traitement</a></span>
            </div>
            </div>
            <?php
        }
    ...
    ?>
       ...
       window.addEventListener('scroll', () => {
         ...
         navdd.style.transition = '0.01s';
           ...
           filterButton1.addEventListener('focus', function() {
             const titre=document.querySelector('.barre-plant h1')
             titre.style.opacity=0
             titre.style.transition="0.1s"
             this.style.left="20%";
       });
       filterButton1.addEventListener('blur', function() {
         const titre=document.querySelector('.barre-plant h1')
             titre.style.opacity=1;
             titre.style.transitionDelay ="0.1s";
         this.style.left="60%";
       });
        ...
        const titre = document.querySelector('.barre-plant h1');
         ...
         if (grid && titre) {
           ...
           for (var i = 0; i < items.length; i++) {
             ...
             var groupe = items[i].getElement().querySelector('.groupe1');
       
             if (titleElement) {
               var title = titleElement.textContent.toLowerCase().replace(/\s/g, ''); 
               var groupe1 = groupe.textContent.toLowerCase().replace(/\s/g, ''); 
       
               if (title.includes(searchTerm) || groupe1.includes(searchTerm)) {
